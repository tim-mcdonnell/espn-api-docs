...
|--------|------|-------------|---------|
| provider_id | INTEGER | Primary key (auto-increment) | 1 |
| provider_name | VARCHAR | Name of the odds provider | DraftKings |
| provider_abbreviation | VARCHAR | Short abbreviation for provider | DK |
| provider_logo_url | VARCHAR | URL to provider logo | https://a.espncdn.com/i/bookmakers/draft-kings-logo.png |
| provider_description | VARCHAR | Brief description of provider | DraftKings Sportsbook |
| provider_official | BOOLEAN | Whether provider is an official partner | true |
| provider_priority | INTEGER | Display priority ranking | 1 |
| active | BOOLEAN | Whether provider is currently active | true |
| created_at | TIMESTAMP | Record creation timestamp | 2023-07-12 14:30:00 |
| updated_at | TIMESTAMP | Record update timestamp | 2023-08-15 09:22:31 |

**Indexing:**
```sql
CREATE INDEX idx_odds_providers_name ON OddsProviders(provider_name);
```
...
| provider_id | INTEGER | Foreign key to OddsProviders | 1 |
| timestamp | TIMESTAMP | Time of odds snapshot | 2024-01-09 22:30:00 |
...
| created_at | TIMESTAMP | Record creation timestamp | 2024-01-09 22:30:00 |
| updated_at | TIMESTAMP | Record update timestamp | 2024-01-09 22:30:00 |

**Indexing:**
```sql
CREATE INDEX idx_provider_event_odds_event ON ProviderEventOdds(event_odds_id);
CREATE INDEX idx_provider_event_odds_provider ON ProviderEventOdds(provider_id);
...
| movement_timestamp | TIMESTAMP | When movement occurred | 2024-01-09 15:30:00 |
| movement_direction | VARCHAR | Direction of movement | toward_home |
| magnitude | DECIMAL | Size of movement | 1.0 |
| created_at | TIMESTAMP | Record creation timestamp | 2024-01-09 15:30:00 |
...
| timestamp | TIMESTAMP | Time of odds snapshot | 2024-01-09 00:00:00 |
| previous_odds | INTEGER | Previous odds value | +900 |
| movement_direction | VARCHAR | Recent movement direction | improved |
| created_at | TIMESTAMP | Record creation timestamp | 2023-12-30 00:00:00 |
| updated_at | TIMESTAMP | Record update timestamp | 2024-01-09 00:00:00 |
...
CREATE INDEX idx_futures_odds_provider ON FuturesOdds(provider_id);
```
...
- `ProviderEventOdds` links to `EventOdds` via `event_odds_id` and to `OddsProviders` via `provider_id`
...
```sql
SELECT e.name AS event_name, e.start_date, eo.consensus_spread, eo.consensus_over_under,
       eo.consensus_home_moneyline, eo.consensus_away_moneyline,
       ht.display_name AS home_team, at.display_name AS away_team
FROM EventOdds eo
JOIN Events e ON eo.event_id = e.event_id
JOIN Teams ht ON eo.home_team_id = ht.team_id
JOIN Teams at ON eo.away_team_id = at.team_id
WHERE e.event_id = 401524661;
```

### Analyzing Line Movement Before Game

```sql
SELECT om.movement_timestamp, om.movement_type, om.old_value, om.new_value,
       om.movement_direction, op.provider_name
FROM OddsMovement om
JOIN OddsProviders op ON om.provider_id = op.provider_id
JOIN EventOdds eo ON om.event_odds_id = eo.event_odds_id
WHERE eo.event_id = 401524661
ORDER BY om.movement_timestamp;
```
